...
module.exports = {
  get: (params) => {
    const table = db.table('wordGroups');
  ...
  create: (wordGroup) => {
    const valid = validate(wordGroup);
    if (!valid) return Promise.reject(valid);
    return db.table('wordGroups').insert(wordGroup, {returnChanges: true}).run()
    .then(firstChange);
  },
  update: (wordGroup) => {
    const valid = validate(wordGroup);
    if (!valid) return Promise.reject(valid);
    return db.table('wordGroups').update(wordGroup, {returnChanges: true}).run()
    .then(firstChange);
  },
  delete: (id) => {
    return db.table('wordGroups').get(id).delete({returnChanges: true}).run()
    .then(res => {
      return res;
    })
    .then(firstChange);
